...
- **共有 API / DTO の型変更で内部スキーマだけを根拠に判断し、他リポジトリの既存クライアント
  実装を確認しなかった**（後追いで横断調査したら歴史的な慣習と乖離していた）
- **既存コードベースに前例のない独自 sentinel 契約（特殊なフォールバック値等）を
  勝手に導入した**（防御コードが「予測可能性を損なう歪み」になった）
- **null を空文字フォールバックで潰した**（「未入力」と「不明」を
  区別不能にした。値が無いものは Optional のまま伝搬するのが原則）
- **プロジェクト固有のブランチ命名規約（Issue 番号必須・接尾辞ルール等）を見落とした**
  （プロジェクトの命名規約ドキュメントを読まずに仮決めした）
...
1. **プロジェクト固有のブランチ命名規約**を必ず確認する。`CONTRIBUTING.md` /
   `.claude/rules/` / `docs/` 配下などに **Issue 番号・接頭辞・接尾辞などの形式** が
   定義されていることが多い。ジェネリックな `fix/foo` 形式で仮決めすると、規約違反で
   PR が拒否されたり後から rename を強いられる
2. タスクから仮のブランチ名を決める。Issue 番号が命名規約に含まれる場合は Phase 2-2 で
   確定するので、**2-1 と 2-2 を統合して順序立て**、Issue 番号確定後にブランチを切る
3. ブランチ名と一文の根拠を宣言する
4. そのまま worktree を作成し、作業ディレクトリとして使う
5. 確認は求めない
...
| **公開 API / 共有データ型の破壊的変更を含む** | 他リポジトリのクライアントと共有する Read Model / DTO / Schema の型変更、フィールドの required ↔ optional 切替、enum 追加削除 | プロジェクト固有のクロスリポ調査スキルがあればそれ。無ければ関連リポジトリごとに 1 体ずつ `Explore` を並列起動 |

### 公開 API / 共有データ型を変更する場合の必須手順

内部スキーマや実装の都合だけを根拠に nullable / required / 型を判断するのは禁止。
**既存クライアント側の実装が「歴史的にどう受けてきたか」が Source of Truth** になる
ケースが多い（特に古参・レガシークライアント）。Phase 3 で以下を**必ず**確認してから
Phase 4 の設計判断に入る:

1. 各クライアント側の対応モデル / DTO の型定義
2. クライアント側で当該フィールドを **null** / 空文字 / 欠落に対してどう扱っているか
   （Optional 受け、防御コード、空文字フォールバックの有無）
3. 過去 6〜12 ヶ月の関連実装変更履歴（同型の修正先例があるか）

「クライアント側で確認できなかった」場合は、**確認できなかったこと自体を Phase 6 報告に
明記**して、ユーザーに判断を委ねる。「内部スキーマ上 nullable だから Optional が
正しいはず」という抽象論で破壊的変更を進めない。
...
| 「内部スキーマ上は理論的に nullable だから防御コードを入れておこう」 | デフォルト値や制約で実運用 null にならないフィールドに防御コードを入れると、独自契約の温床になる。実データで起きないなら受信側を非 Optional にして、万一 null が来たら 5xx で異常を可視化する方が安全 |
| 「ブランチ名は `fix/foo-description` で OK だろう」 | プロジェクトの命名規約（`CONTRIBUTING.md` / `.claude/rules/` 等）で **Issue 番号や接尾辞を含む形式** が必須なケースが多い。Phase 2-1 で必ず確認 |
| 「先例の実装ファイルがこうだから合わせる」 | 先例を盲信しない。先例自体が改善余地を持つ可能性がある。**本件で正しい設計は何か** を独立に判断し、先例と異なる選択をするなら理由を明記 |
