...
検出方法（優先順）:
1. 利用可能な subagent_type から `reviewer` を含むプロジェクト固有エージェントを検出（汎用3エージェントとプラグインは除外）
2. フォールバック: `.claude/agents/*reviewer*.md` を Glob 検索（description に `respond-review用` / `Agent Teams用` を含むものは除外）

検出した場合のみ「プロジェクト固有レビューエージェントを検出: <名前一覧>」と表示する。
...
再レビューのサマリーは初回より短く、あっさりと書く。mustがなければ1行で済ませる。例:
> LGTM、修正ありがとうございます!
...
> 読みやすい実装ですね。テストも丁寧で助かります。（印象から入る）
> キャッシュ戦略の見直し、良い判断だと思います。（変更の核心から入る）
> LGTM 🎉（端的に評価する）
> 並行処理周りで修正が必要そうなところがあります。（気になる点から入る）
...
`post_approval_mode` + mustなしの場合: 「問題なし、GitHubへの投稿なし」旨のみ報告。
