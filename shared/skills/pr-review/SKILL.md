...
### 2-3. プロジェクト固有レビューエージェントの検出（必須ステップ — スキップ不可）

汎用エージェント（correctness-reviewer, security-perf-reviewer, quality-test-reviewer）やプラグインエージェント（pr-review-toolkit:* 等）以外に、プロジェクト固有のレビューエージェントを検出し、Phase 4 で必ず起動する。
...
3. 他リポジトリのPRレビュー時: プライマリ作業ディレクトリの `.claude/agents/` も確認する（例: SPIDERPLUS-web のPRレビューでも SP-ACL の spec-reviewer を使う）

検出結果を必ず表示する: 「プロジェクト固有レビューエージェントを検出: <名前一覧>」または「プロジェクト固有レビューエージェント: なし」。
...
以下の観点で確認する。全観点を機械的にチェックするのではなく、PRの文脈に合った観点に集中する。

**仕様整合性**
- PR本文・関連Issueの要件が実装に反映されているか
- 要件にない過剰実装（スコープクリープ）がないか
- 実装アプローチは要件に対して妥当か
...
以下の4つのエージェントを **可能な限り同時に** 起動する:

| エージェント名 | 専門性 | 担当観点 |
|---|---|---|
| `specification-reviewer` | 仕様・要件の整合性を検証するエンジニア | 仕様整合性・設計判断 |
| `correctness-reviewer` | コードの正しさと堅牢性を検証するエンジニア | 正しさ・堅牢性 |
| `security-perf-reviewer` | セキュリティ・パフォーマンス・運用の専門家 | セキュリティ + パフォーマンス + 運用 |
| `quality-test-reviewer` | テスト品質とコード保守性のエキスパート | テスト + 可読性・技術的負債 |
...
> 問題ありません。マージしてOKです。テストも十分です。（判断から入る）
> キャッシュ戦略の見直し、設計・実装ともに良いです。（変更の核心から入る）
> LGTM 🎉（端的に評価する）
> 並行処理周りに修正が必要です。修正してからマージしましょう。（修正要求から入る）
