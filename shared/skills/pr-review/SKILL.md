...
抽出できない場合は `AskUserQuestion` でPR番号を確認する。
...
変更ファイルのうち、特に以下のケースでは周辺コードをReadで確認する:
...
- `CLAUDE.md` / `.claude/settings.json` — プロジェクトのコーディング規約
...
以下の3つの Agent を **1つのメッセージで同時に** 起動する:
...
- `name`: 上記エージェント名（`$CLAUDE_CONFIG_DIR/agents/` に定義済み）
...
- サマリーとインラインコメントは別物。サマリーで個別指摘の内容を繰り返さない
...
2. 重要な指摘のみ本文で触れる — mustは必ず言及、suggestionは重要なもののみ。nit/goodはインラインに任せる
3. 着眼点は議論が必要な場合のみ記載する
4. 件数の統計表は書かない
...
mustがある場合:
> 設計の方向性は良いと思います。ただ、2点ほど修正が必要そうなところがあります。
>
> `handleError` でエラーを握りつぶしているのが気になります。ここでログも出ないと、本番で問題が起きたとき追えなくなりそうです。
>
> 細かい点はインラインで書きました。

mustがない場合:
> いい感じだと思います。いくつか改善案をインラインで書きました。
