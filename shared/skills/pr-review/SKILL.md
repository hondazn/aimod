...
- 「結果が出揃った」「サマリーとして十分な情報量がある」のような感覚的判断で停止しない。完了条件は冒頭「完了条件 (Definition of Done)」で定義した通り、投稿 URL の報告または正規早期終了理由の明示報告である

非対話的に実行されている場合も同じ。中間出力（findings 表、エージェントの raw 出力など）を返したことをもって完了報告に代えてはならない。
