...
### Step 1: コミットメッセージフォーマットの決定

上記の `git log` の出力から、リポジトリのコミットメッセージの慣習を分析する。
...
- **構造と振る舞いは分割**: リファクタリング・整理（構造の変更）と機能追加・修正（振る舞いの変更）は別コミットにする
...
Step 1で決定したフォーマットに従ってメッセージを作成する。
...
- サマリーは50字以内、本文を書く場合は72字で折り返す（50/72ルール）
