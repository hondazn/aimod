---
allowed-tools: Bash(codex:*) Bash(gh:*) Bash(git:*) Bash(cat:*) Bash(ls:*) Bash(mktemp:*) Read(*) Glob(*) Grep(*)
argument-hint: '[調査テーマ 例: 認証モジュールの構造を調査して / Issue #123の原因を調べて]'
description: |
    コードベースの調査・プランニング・設計検討を Codex CLI（GPT-5.4）に委譲するスキル。read-only モードでリポジトリを探索し、調査結果を Claude が咀嚼・圧縮して提示する。
    トリガー: 「調査して」「調べて」「リサーチして」「プランニングして」「計画を立てて」「実装計画を作って」「原因を調べて」「根本原因を特定して」「影響範囲を調べて」「アーキテクチャを調査して」「設計を検討して」「どう実装されているか調べて」「コードベースの〜を調べて」「〜の仕組みを調査して」。Issue 番号や URL 付きの調査依頼にも対応する。
    codex:codex-rescue は Claude が行き詰まった時のリアクティブツールだが、このスキルはユーザーの調査・計画リクエストに最初から Codex を使うプロアクティブな委譲スキル。
