...
| 非対話実行の「完了」通知が早すぎる | `nohup ... &` で起動すると、ハーネスが完了と見なすのは**起動コマンドの完了**であって本体の終了ではない。出力ファイルを読むと作業途中で、判定が出ていないのに終わったと誤認する。**プロセスの生存（`pgrep -f`）と判定行の出現の両方**で待つこと。`pgrep` のパターンが自分のシェル行にも一致して偽陽性になるので、出力ファイルの更新停止も併せて見る |
| opencode が途中で止まり判定を出さない | `--dir` の外への書込を自動拒否し、**拒否されると適応せず停止する**。プロンプトに「`--dir` の外には書けない」制約と代替手順（作業ディレクトリ内で完結する方法）を明記する。レビューなら使い捨てコピーを `--dir` に指定し、コピー内を `git init` して「注入 → テスト → `git checkout -- .`」を手順として渡す |
